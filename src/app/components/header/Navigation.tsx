...
const navItems = [
    { icon: faHouse, label: "HOME", mt: "mt-24" },
    { icon: faMap, label: "MAP", mt: "mt-4" },
    { icon: faRectangleList, label: "LIST", mt: "mt-4" },
];
    ...
    return(
        <div className={`w-64 h-screen fixed top-0 ${nabFlag ? '-left-64' : 'left-0'} z-20 flex flex-col transition-all duration-500`} style={{backgroundColor:mainColor}}>
            {navItems.map((item, index) => (
                <div key={index} className={`flex items-center gap-4 ${item.mt} ml-8`}>
